...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const checkHealthAndLoad = async () => {
      try {
        const response = await fetch("/api/astro/health", {
          headers: {
      ...
      {isBooting && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black text-white">
          <div className="flex w-full max-w-md flex-col items-center gap-6 px-6 text-center">
            ...
            {apiHealthy === false ? (
              <div className="w-full rounded-3xl bg-white/[0.06] px-6 py-6 shadow-[0_0_60px_rgba(255,255,255,0.08)]">
                <p className="text-[0.7rem] uppercase tracking-[0.45em] text-white/45">
                  Sistema offline
                </p>
                <p className="mt-3 text-xl font-semibold text-white">
                  A API ainda não foi inicializada
                </p>
                <p className="mt-3 text-sm leading-6 text-white/65">
                  Para continuar, finalize a instalação na tela inicial e inicie
                  a API.
                </p>
                <Link
                  href="/#install"
                  className="mt-5 inline-flex items-center justify-center rounded-full bg-white px-5 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-black transition hover:-translate-y-0.5 hover:bg-white/90"
                >
                  Ver instalação
                </Link>
              </div>
          ...
          <header className="flex flex-col gap-4 border-b border-white/10 pb-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl animate-reveal [animation-delay:60ms]">
              ...
              <h1 className="font-display text-4xl leading-[0.95] tracking-tight text-white sm:text-5xl lg:text-7xl">
                Painel astronômico para rastrear céu, visibilidade e satélites
                em tempo real.
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-white/68 sm:text-base">
                Uma central cósmica com vidro translúcido, leituras ao vivo e
                foco em análise rápida para objetos celestes, objetos visíveis e
